# Estimates HVAC parameters based on benchmark scenario
...
# Specify where data of benchmark run is located
# Benchmark run: year-long glm run without market (i.e. don't include gridlabd_functions module in glm file)

run = 'Test'
ind_base = 0 # 90 # no PV 46 # 10 % PV
use_existing_beta = False
city = 'Austin'
market_file = 'Ercot_LZ_SOUTH.csv'

start = pd.Timestamp(2016,1,4) # First Monday in the year
end = pd.Timestamp(2016,1,10,23,55) # First Sunday in the year
...
#Total load
df_total_load_year = pd.read_csv(folder+'/total_load_all.csv',skiprows=range(8))
df_total_load_year['# timestamp'] = df_total_load_year['# timestamp'].map(lambda x: str(x)[:-4])
df_total_load_year['# timestamp'] = pd.to_datetime(df_total_load_year['# timestamp'])
df_total_load_year.set_index('# timestamp',inplace=True)
...
# Unresp load forecast
df_slack['hvac'] = df_hvac_load_year.sum(axis=1)
df_slack['baseload'] = df_slack['measured_real_power'] - df_slack['hvac']
df_unresp_load_agg = pd.DataFrame(index=df_slack.index,columns=['baseload'],data=df_slack['baseload'].values)
df_unresp_load_agg.to_csv('glm_generation_'+city+'/'+'perfect_baseload_forecast_'+str(ind_base)+'.csv')
...
df_T_out_year = df_T_out_year.iloc[:-1]
